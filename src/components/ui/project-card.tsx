...
const ProjectCard: React.FC<{ project: Project, className?: string }> = ({ project, className }) => {
    ...
    return (
        <div
            className={cn("col-span-1 sticky max-sm:top-16 top-20 duration-300 overflow-hidden p-0.5 max-sm:rounded-none bg-card group font-sans rounded-4xl mb-4 ring-1 ring-border hover:ring-primary/50 hover:ring-2 transition-all ease-in-out flex flex-col h-full", className)}
            role="article"
            tabIndex={-1}>
